...
def load_data():
    if os.path.exists(DATA_PATH):
        ...
        return pd.DataFrame(columns=["اسم المهمة", "الكمية", "العدد", "سعر الوحدة", "التكلفة"])

def calc_cost(row):
    return row["الكمية"] * row["سعر الوحدة"] * row["العدد"]
...
with st.form("task_form"):
    ...
    if submitted:
        df = load_data()
        cost = quantity * unit_price * count
        df.loc[len(df)] = [name, quantity, count, unit_price, cost]
...
if not df.empty:
    ...
    st.markdown(f"### 💰 المجموع الكلي: {total:,.2f} ريال")
    area = st.number_input("📐 المساحة الكلية بالمتر المربع", min_value=1.0)
    if area:
        cost_per_meter = total / area
        st.markdown(f"### 💸 تكلفة المتر المربع: {cost_per_meter:,.2f} ريال")
